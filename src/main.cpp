...
#if defined(NDEBUG)
#error "Midori cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zMidori spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zMidori spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zMidori spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 2520000 * COIN;
    } else if (nHeight < 28801 && nHeight > 0) {
        nSubsidy = 1 * COIN;
    } else if (nHeight < 57602 && nHeight > 28800) {
        nSubsidy = 5 * COIN;
    } else if (nHeight < 115203 && nHeight > 57601) {
        nSubsidy = 15 * COIN;
    } else if (nHeight < 158404 && nHeight > 115202) {
        nSubsidy = 20 * COIN;
    } else if (nHeight < 187205 && nHeight > 158403) {
        nSubsidy = 25 * COIN;
    } else if (nHeight < 201606 && nHeight > 187204) {
        nSubsidy = 30 * COIN;
    } else if (nHeight < 230407 && nHeight > 201605) {
        nSubsidy = 25 * COIN;
    } else if (nHeight < 273608 && nHeight > 230406) {
        nSubsidy = 20 * COIN;
    } else if (nHeight < 2233930 && nHeight > 273607) {
        nSubsidy = 10 * COIN;
    } else if (nHeight < 4194251 && nHeight > 2233929) {
        nSubsidy = 5 * COIN;
    } else if (nHeight < 6154572 && nHeight > 4194250) {
        nSubsidy = 2.5 * COIN;
    } else if (nHeight < 8601652 && nHeight > 6154571) {
        nSubsidy = 1.25 * COIN;
    } else if (nHeight < 42000001 && nHeight > 8601651) {
        nSubsidy = 1.25 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
    ...
    if (nHeight < 201) {
        ret = blockValue * 0;
    } else if (nHeight < 28801 && nHeight > 200) {
        ret = blockValue * 0.85;
    } else if (nHeight < 57602 && nHeight > 28800) {
        ret = blockValue * 0.85;
    } else if (nHeight < 115203 && nHeight > 57601) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Midori
...
{
    RenameThread("midori-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZMDRMinted()
{
...
void RecalculateZMDRSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zMDR supply
...
bool RecalculateMDRSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zMidori with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zMidoriSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert MDR to zMDR
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zMidori spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zMidori serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zMidori mints and %d zMidori spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Midori: We use certain sporks during IBD, so check to see if they are
