...
#include "ui_zmidoricontroldialog.h"
...
std::list<std::string> ZMidoriControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZMidoriControlDialog::listMints;

ZMidoriControlDialog::ZMidoriControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZMidoriControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZMidoriControlDialog::~ZMidoriControlDialog()
{
    delete ui;
}

void ZMidoriControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZMidoriControlDialog::updateList()
{
...
void ZMidoriControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZMidoriControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZMidori_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZMidoriControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZMidoriControlDialog::GetSelectedMints()
{
...
void ZMidoriControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
