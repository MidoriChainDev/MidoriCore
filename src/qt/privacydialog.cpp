...
#include "zmidoricontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zMDR ought to be enough for anybody." - Bill Gates, 2017
    ui->zMDRpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzMDRSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzMDR->setEnabled(false);
        ui->pushButtonMintzMDR->setToolTip(tr("zMDR is currently disabled due to maintenance."));

        ui->pushButtonSpendzMDR->setEnabled(false);
        ui->pushButtonSpendzMDR->setToolTip(tr("zMDR is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zMDRpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzMDR_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMDR is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zMDR...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zMDR in ") +
...
void PrivacyDialog::on_pushButtonSpendzMDR_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zMDR is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zMDR
        sendzMDR();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zMDR
    sendzMDR();
}

void PrivacyDialog::on_pushButtonZMidoriControl_clicked()
{
    ZMidoriControlDialog* zMidoriControl = new ZMidoriControlDialog(this);
    zMidoriControl->setModel(walletModel);
    zMidoriControl->exec();
}

void PrivacyDialog::setZMidoriControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzMidoriSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzMDR()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Midori Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zMDRpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zMDR is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " MDR </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zMDR</b>";
    ...
    // use mints from zMidori selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZMidoriControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZMidoriControlDialog::GetSelectedMints();
    }
    ...
    // Clear zmidori selector in case it was used
    ZMidoriControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zMidori Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Midori, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zMidori Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zMDR </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zMDR "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zMDR "));
    ui->labelzMDRAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzMDRSyncStatus->setVisible(fShow);
}
